...
>
> ⚠️ **2026-08-06 更新**：区块链栈已与数据栈统一部署至**单机 `43.163.105.172`**（旧服务器 ~~43.156.99.215 / 129.226.203.60~~ 弃用），公网统一经 nginx（80/443），下表服务端口不变。数据栈部署见 [docs/DEPLOYMENT_DATA_STACK.md](./docs/DEPLOYMENT_DATA_STACK.md)。
...
**生产地址**: 单机 `43.163.105.172`（数据栈 + 区块链栈 + admin/web + nginx 80/443 同机）  
**SSH**: `ssh ubuntu@43.163.105.172`（直连，不需要跳板机）  
**密码**: 由运维管理
