...
function encodePageToken(cursor: { block_number: number; event_id: string }): string {
  return Buffer.from(JSON.stringify(cursor)).toString('base64');
}
function decodePageToken(token: string): { block_number: number; event_id: string } | null {
...
// ─── B-end Data Query Endpoints (require x-dc-api-key) ───

app.get('/api/v2/data/events', requireDcApiKey, asyncHandler(async (req: any, res: any) => {
  const pageSize = Math.min(parseInt(req.query.page_size) || DEFAULT_PAGE_SIZE, MAX_PAGE_SIZE);
  ...
  if (req.query.page_token) {
    const cursor = decodePageToken(req.query.page_token);
    if (cursor) { conditions.push(`(block_number < $${idx} OR (block_number = $${idx} AND event_id > $${idx+1}))`); values.push(cursor.block_number, cursor.event_id); idx += 2; }
  }

  const where = conditions.length > 0 ? `WHERE ${conditions.join(' AND ')}` : '';
  const query = `SELECT event_id, event_type, chain, block_number, tx_hash, from_address, to_address, contract_address, token_address, token_symbol, amount, amount_raw, confirmations, collected_at, created_at FROM events ${where} ORDER BY block_number DESC, event_id ASC LIMIT $${idx}`;
  ...
  if (rows.length > pageSize) {
    ...
    next_token = encodePageToken({ block_number: parseInt(last.block_number), event_id: last.event_id });
  }
...
app.get('/api/v2/data/stats', requireDcApiKey, asyncHandler(async (_req: any, res: any) => {
  const statsResult = await pool.query(`SELECT chain, COUNT(*)::int as event_count, MAX(block_number)::bigint as latestBlock, COUNT(DISTINCT tx_hash)::int as uniqueTx FROM events GROUP BY chain ORDER BY event_count DESC`);
  const totalResult = await pool.query('SELECT COUNT(*)::int as cnt FROM events');
  res.json(apiResponse({ chains: statsResult.rows, totalRows: totalResult.rows[0].cnt }));
}));

app.get('/api/v2/data/health', requireDcApiKey, asyncHandler(async (_req: any, res: any) => {
  const totalResult = await pool.query('SELECT COUNT(*)::int as cnt FROM events');
  const checkpoints = await pool.query('SELECT chain, collector_name, last_block, status, last_fetch_at FROM event_checkpoints ORDER BY chain');
  res.json(apiResponse({ status: 'ok', totalEvents: totalResult.rows[0].cnt, checkpoints: checkpoints.rows }));
}));

app.get('/api/v2/data/checkpoints', requireDcApiKey, asyncHandler(async (_req: any, res: any) => {
  const result = await pool.query('SELECT chain, collector_name, last_block, status, last_fetch_at FROM event_checkpoints ORDER BY chain');
...
app.get('/health', asyncHandler(async (_req: any, res: any) => {
  res.json({ status: 'ok', service: 'infrax-c', uptime: process.uptime() });
}));
