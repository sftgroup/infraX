"""Adanos Market Sentiment collector — optional provider for US stock tickers.

Ports the legacy ``data_providers/adanos_sentiment.py`` into the data-service
collector architecture. Intentionally fail-open: when ``ADANOS_API_KEY`` is not
configured the snapshot is saved with ``enabled=false`` instead of raising,
keeping data-service fully usable without Adanos.
"""
...
# Default watchlist; override via ADANOS_TICKERS (comma-separated).
_DEFAULT_TICKERS = os.getenv(
    "ADANOS_TICKERS", "AAPL,MSFT,NVDA,TSLA,AMZN,META,GOOGL,PLTR,COIN,MSTR"
)
COLLECT_INTERVAL = int(os.getenv("ADANOS_COLLECT_INTERVAL_SEC", "3600"))  # 1 h
...
def fetch_adanos_market_sentiment(
    ...
    *,
    ...
    """Fetch optional Adanos Market Sentiment snapshots (fail-open)."""
...
class AdanosCollector:
    """Periodically fetch Adanos market sentiment and write to raw_snapshots."""
    ...
    def start(self):
        if self._running:
            return
        self._running = True
        self._thread = threading.Thread(target=self._loop, daemon=True, name="adanos-collector")
        self._thread.start()
        logger.info("AdanosCollector started (interval=%ds)", COLLECT_INTERVAL)
    ...
    def _loop(self):
        while self._running:
            try:
                ...
                logger.warning("Adanos collector cycle failed", exc_info=True)
            time.sleep(COLLECT_INTERVAL)

    def _collect(self):
        data = fetch_adanos_market_sentiment(_DEFAULT_TICKERS)
        if data.get("error") and not data.get("enabled"):
            logger.debug("Adanos disabled: %s", data.get("error"))
        save_snapshot("sentiment", "adanos_sentiment", data)
        logger.info(
            "Adanos snapshot saved: enabled=%s stocks=%d",
            data.get("enabled"), len(data.get("stocks") or []),
        )
