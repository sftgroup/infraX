...
def fetch_vix() -> Dict[str, Any]:
    ...
    Chain: Finnhub VXX (fastest) → yfinance → akshare.
    Each external call is short-timeout. The function always returns within
    ~12 s even when upstream APIs stall, falling back to a sensible default.
    """
    DEFAULT_VIX = {
    ...
    try:
        ...
        if key:
            ...
            vxx = json.loads(resp.read())
            if vxx.get("c"):
                current = float(vxx["c"])
                change = float(vxx.get("dp", 0))
                logger.info("VIX from Finnhub VXX: %.2f", current)
    except Exception as e:
        logger.debug("Finnhub VXX skipped: %s", e)
        ...
        try:
            ...
            if hist is not None and not hist.empty and len(hist) >= 1:
                ...
                if current > 0:
                    ...
                    logger.info("VIX from yfinance: %.2f", current)
        except Exception as e:
            logger.debug("yfinance VIX failed: %s", e)

    # Tier 3: akshare (try multiple method names)
    if current <= 0:
        try:
            import akshare as ak
            vix_df = None
            for mname in ("index_option_vix", "index_us_stock_vix", "index_vix"):
                m = getattr(ak, mname, None)
                if m is None:
                    continue
                try:
                    vix_df = m() if callable(m) else None
            ...
            logger.debug("Akshare VIX failed: %s", ak_err)
...
def fetch_dollar_index() -> Dict[str, Any]:
    ...
    Sources: East Money (primary) → akshare USDRND (fallback) → default.
    """
    DEFAULT_DXY = {
    ...
    # Tier 1: East Money global index spot (real DXY from UDI symbol)
    try:
        import akshare as ak
        ...
        for _, row in df.iterrows():
            ...
            if code == "UDI":
                ...
                logger.info("DXY from East Money: %.2f", current)
                break
    except Exception as em_err:
        logger.warning("East Money DXY failed: %s", em_err)

    # Tier 2: akshare forex estimate (fallback)
    if current <= 0:
        try:
            import akshare as ak
            ...
            if fx_df is not None and len(fx_df) > 0:
                ...
                change = 0
                logger.info("DXY estimated from akshare USDCNY: %.2f", current)
        except Exception as ak_err:
