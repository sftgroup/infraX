...
def _fetch_yahoo_urlopen_quote(symbol: str) -> Optional[Dict[str, Any]]:
    """US spot quote via Yahoo chart API using urlopen — bypasses requests/urllib3
    which may be IP-blocked in Docker. urlopen is the same transport Finnhub uses."""
    sym = (symbol or "").strip().upper()
    if not sym:
        return None
    try:
        ...
        req = Request(url, headers={
            "User-Agent": "Mozilla/5.0 (compatible; QuantDinger/1.0)",
...
def _fetch_single_local_stock_quote(market: str, item: Dict[str, Any], *, fast: bool = False) -> Optional[Dict[str, Any]]:
    ...
    if m == "USStock":
        ...
        if _finnhub_key:
            ...
            if fh:
                last = safe_float(fh.get("last"))
                change_pct = safe_float(fh.get("changePercent"))
        # Fallback 1: Yahoo Finance (free, but rate-limited → add stagger)
        if last <= 0:
        ...
        if last <= 0:
            _time.sleep(0.4)  # stagger requests to avoid Yahoo 429
        ...
        if last <= 0:
            _time.sleep(0.4)
        # Fallback 3: Yahoo Finance via urlopen (bypasses requests/urllib3 blocking)
        if last <= 0:
        ...
        if last <= 0:
            _time.sleep(0.3)
        # Fallback 4: CNBC scraper (free, no auth, works globally)
        if last <= 0:
            from app.data_providers.cnbc_scraper import fetch_cnbc_quote
            cnbc = fetch_cnbc_quote(symbol)
            if cnbc:
                last = safe_float(cnbc.get("last"))
                change_pct = safe_float(cnbc.get("changePercent"))
        if last <= 0:
            _time.sleep(0.4)
        # Fallback 5: DataSourceFactory (same path as HK/CN stocks, uses yfinance → likely blocked)
        if last <= 0:
        ...
        # Fallback 5: KlineService (skip in fast mode — slow)
...
def fetch_local_stock_opportunity_prices(market: str, limit: int = 15, *, fast: bool = False) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if m in ("USStock", "HKStock", "CNStock") and len(items) > 1:
            workers = min(3, len(items))  # reduced from 8 to avoid triggering Yahoo 429 rate limits
