...
> 版本 1.0 | 端口 8765 | FastAPI + SQLite

## 1. 功能总览

Data Service 为整个 AItrader 平台提供**多市场行情数据**和**技术/基本面因子**，所有数据统一存储在本地 SQLite 中，通过 REST API 对外提供。
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `DATA_SERVICE_PORT` | 8765 | 服务端口 |
| `KL_SYMBOLS` | BTC/USDT,ETH/USDT,SOL/USDT | 加密K线标的 |
...
│  ┌───────────────────────────┐  │
│  │  DataClient  (HTTP :8765) │──┼──→ Data Service Container
│  └───────────────────────────┘  │
└─────────────────────────────────┘
```
...
**模式 B — 独立服务**
```
Your Platform → HTTP → data-service:8765/bars?symbol=...
                      data-service:8765/factors/current
                      data-service:8765/snapshots?type=heatmap
```
...
# 热力图
resp = requests.get(f"{BASE}/snapshots", params={"type": "heatmap"})
...
3. **API Key 可选** — 未配置 CoinGecko/Finnhub/FRED Key 时对应采集器自动降级或跳过
