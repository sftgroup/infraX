/**
 * B-10-6: vault 链上访问统一收敛到 chain-rpc 网关（禁止直连上游 RPC）。
 *
 ...
 * 不直连任何上游 RPC，全部由网关汇总分发（对齐 waas/mpc 的 GatewayProvider 模式）。
 */
...
export class GatewayProvider extends ethers.JsonRpcProvider {
  ...
  override async send(method: string, params: Array<any>): Promise<any> {
    ...
    // 广播端点契约：{ rawTransaction, wait }；读端点契约：{ method, params }。
    ...
    try {
      ...
      throw new Error(`[vault-gateway] non-json response ${resp.status} from ${url}`);
    }
    if (resp.status !== 200 || json.code !== 0) {
      throw new Error(`[vault-gateway] ${json.detail || json.message || `http ${resp.status}`} (method=${method})`);
    }
