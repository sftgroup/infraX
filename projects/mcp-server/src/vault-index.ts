...
async function handle(req: any) {
  ...
  try {
    if (method === 'initialize')
      return { jsonrpc: '2.0', id, result: { protocolVersion: '2024-11-05', capabilities: { tools: {} }, serverInfo: { name: 'infrax-vault-mcp', version: '1.0.0' } } };
...
app.get('/health', (_q, res) => res.json({ status: 'ok', service: 'infrax-vault-mcp', tools: Object.keys(tools).length }));
