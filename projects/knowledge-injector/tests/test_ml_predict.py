"""providers/ml.py 纯函数单测（Kronos 波动率预测）。
...
依赖：numpy/pandas（providers/ml.py 模块级 import）。
"""
...
# 强制测试环境 KRONOS_ENABLED=false（config 实例化在 import 时完成）
...
class TestVolLevel:
    def test_boundaries(self):
        assert ml._vol_level(0.0) == "very_low"
        assert ml._vol_level(0.24) == "very_low"
        assert ml._vol_level(0.25) == "low"
        assert ml._vol_level(0.40) == "moderate"
        assert ml._vol_level(0.55) == "high"
        assert ml._vol_level(0.70) == "very_high"
        assert ml._vol_level(0.99) == "very_high"

    def test_mid_values(self):
        assert ml._vol_level(0.1) == "very_low"
        assert ml._vol_level(0.3) == "low"
        assert ml._vol_level(0.5) == "moderate"
        assert ml._vol_level(0.6) == "high"


class TestPathStats:
    def test_empty_paths(self):
        assert ml._path_stats([], 100.0) == {}

    def test_non_positive_last_close(self):
        paths = [np.array([100.0, 105.0])]
        assert ml._path_stats(paths, 0) == {}

    def test_convergent_low_vol_high_consensus(self):
        # 所有路径几乎同涨：低波动 + 强方向共识
        paths = [np.array([101.0, 103.0, 105.0]) for _ in range(5)]
        stats = ml._path_stats(paths, last_close=100.0)
    ...
    def test_score_range(self):
        ...
        for p in paths:
            s = ml._path_stats([p], last_close=100.0)
...
class TestDisabledBehavior:
    ...
    def test_predictor_not_loaded(self):
        assert ml._load_predictor() is None
        assert ml._predictor is None
