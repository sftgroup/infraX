...
if (!ADMIN_PASS) {
  console.error('ADMIN_PASS environment variable is not set. Server refused to start.');
  process.exit(1);
}
...
app.get('/api/v2/admin/revenue', requireAdmin, asyncHandler(async (_req: any, res: any) => {
  const [activeTenants, dcSubscribers, subscriptions, payments30d] = await Promise.all([
    pools.waas.query("SELECT COUNT(*)::int as cnt FROM tenants WHERE status='active'").then(r => r.rows[0]?.cnt || 0).catch(() => 0),
