...
# payment engine db (infrax_payments, migration 004), not the collector db.
PAY_DELETED=$($PG -d infrax_payments -t -A -c \
...
OKX_DELETED=$($PG -d infrax_collector -t -A -c \
...
BNB_DELETED=$($PG -d infrax_collector -t -A -c \
