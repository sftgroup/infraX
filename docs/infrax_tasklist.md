...
| W-8 | Chain RPC 页面进入后看不到套餐选择，无法订阅/获取 key（`rpcInit` 仅服务介绍，注释明确"套餐与用量请通过管理端查询"） | ① 后端新增 `walletAuth` 中间件（ethers EIP-191，对齐 `"InfraX auth: <ts>"` 契约，TTL 24h）② `rpc_keys` 表新增 `wallet_address` 列 + `idx_rpc_keys_wallet` 索引（ALTER 兼容已有表）③ 新增 `POST /v1/subscription/wallet-issue-key`（钱包幂等签发 rx_ key：每钱包最多 1 个 active，明文仅签发时展示一次，重复签发返回掩码）与 `GET /v1/subscription/wallet-me`（本人 keys 掩码 + 套餐 + 当月用量 + 配额进度）④ 前端 `rpcInit` 新增套餐卡片区（拉取 `/v1/subscription/plans` 真实价格/配额）+「我的订阅」区：未连钱包引导 connect；已连展示 keys/用量进度条；免费一键签发即激活、付费走 `checkout`（chain 轮询 `payment-check` / fiat 跳转 / x402 提交 txHash），对齐 DC 订阅支付流程 | [b2b.js](projects/web/modules/b2b.js) · [index.html](projects/web/index.html) · [rpcSubscriptionRoutes.ts](projects/chain-rpc/src/routes/rpcSubscriptionRoutes.ts) · [walletAuth.ts](projects/chain-rpc/src/middleware/walletAuth.ts) · [rpcSubscription.ts](projects/chain-rpc/src/services/rpcSubscription.ts) |

**验证**：`node --check` 通过；本地 :6100 起服后浏览器实测 landing.html（统计 14+/10/7、9 张产品卡、7 张 MCP 卡）与 index.html（无 JS 报错、侧边栏 11 项导航）均正常；topbar 链切换全流程（默认 Sepolia → 展开 10 链下拉 → 选中 BSC 标签更新/下拉关闭/toast/持久化 → 重新打开 BSC ✓）实测通过、console 零报错。W-7：WaaS 4 个链下拉（Tokens/Addresses/Sweep/Withdrawals，10 选项）+ Safe 链下拉（4 选项）存在且初始值=activeChain；`px_chain=bsc` 持久化恢复后 topbar 与全部下拉同步为 BSC（token 下拉 = 56）；UI 切 Arbitrum → 4 个 WaaS 下拉同步（token=42161），safe-chain 因无该链选项保持原值（符合 vault 4 链限制）；UI 切 Base → 含 safe-chain 全部同步（token=8453）；console 零报错。W-8：chain-rpc `tsc --noEmit` 通过；本地起服实测 `/v1/subscription/plans` 返回真实套餐目录、`wallet-me`/`wallet-issue-key` 无签名/伪签名均 401、有效签名放行进入 DB 层；生产部署（commit `0f337b4`）后 `rpc_keys` 表含 `wallet_address` 列 + `idx_rpc_keys_wallet` 索引，公网端到端实测：钱包签发返回明文 `rx_` key（keyId 6, rpc_free, active）→ `wallet-me` 返回掩码 + 套餐 + 用量 0/10000 → 重复签发幂等返回 `alreadyExists`（明文不落库）；测试数据已清理，生产服务 active、`/health` 正常。W-8b（布局收尾，commit `d5e0ba9`）：应反馈把「链上事件 · DC 解析增强」面板移出首页改为独立 tab——rpcInit 改为 `.tab-row`（🔑 订阅 / 🔎 链上事件）+ 两个 `sub-panel`（`sub-rpc-sub` 套餐与我的订阅、`sub-rpc-events` 事件查询），首页只保留订阅内容；`rpcSubscribe`/`rpcIssueKey` 未连接钱包时由"仅 toast"改为跳转 `/connect.html`。playwright 生产实测：首页 tab 结构与 3 张套餐卡片渲染正确、事件面板默认隐藏；点击「链上事件」tab → `sub-rpc-events` 激活、`sub-rpc-sub` 隐藏；未连接点击 Get Started → URL 跳转 `https://infrax.0xainet.top/connect.html`。另：为解决 headless Chromium 渲染中文崩溃（SkFontMgr FontConfigInterface Not implemented，环境缺 fontconfig 扫描），创建了用户级 `~/.config/fontconfig/fonts.conf` 指向 `~/.fonts`（含 Noto Sans CJK SC），`FONTCONFIG_PATH` 传入后验证通过。W-8c（内容页补足，commit `f4696f5`）：应反馈"Chain RPC 模块没有内容页"——参照 DC 模块内容页结构，Chain RPC 由 2 个 tab 扩为 4 个内容页：🔑 订阅（套餐卡片+我的订阅）、🔎 链上事件（DC 解析增强查询）、📊 节点状态（`GET /v1/status` 各链 RPC 池端点健康：chainId / active-total / provider / tier / healthy-degraded-down，经反代自动带 bridge key）、📖 API Docs（认证 rx_/bx_、端点表、Quick Start curl 信封+标准 JSON-RPC+ethers v6+viem、Broadcast wait 确认、套餐与限制+错误码）；修复 rpcDocsHtml 误用未定义的 `.code-block`/`.info-table` 改 `.dc-code-block`/`.dc-api-table`。playwright 生产实测：4 个 tab 全部渲染（rpc-sub/rpc-events/rpc-status/rpc-docs），节点状态真实加载 `sepolia chainId 11155111 · 2/4 active` + 端点表（sepolia-tenderly public free ● healthy），API Docs 含 Authentication/ethers v6/viem 文档；缓存版本号 bump 1787272200。W-8d（两阶段订阅模式重构，仿照 DC 模块）：应反馈"仿照其他模块逻辑——点开始介绍+订阅套餐，订阅后进入详情页获取 key 和查看功能状态"——`rpcInit` 由单页平铺改为两阶段容器：`rpc-intro`（介绍 hero + 健康 + 特性 + 接入方式 + 3 张套餐卡 + 🚀 Activate 按钮 + 订阅状态区）与 `rpc-dash`（初始隐藏：KPI 行 Plan/API Calls/Monthly Quota/节点活跃 + 🔑 API Key 面板 + 4 个功能 tab：📋 我的订阅 / 🔎 链上事件 / 📊 节点状态 / 📖 API Docs）；新增 `rpcRefreshSub()` 查询 `wallet-me`：未连钱包 → intro 显示 connect CTA +「直接查看节点状态（无需订阅）」；已订阅（有 key）→ 自动 `rpcLoadDashboard()` 隐藏 intro 进入详情页；未订阅 → 停留 intro 选套餐；`rpcSubscribe`（免费直通 / 付费 checkout 后）与 `rpcIssueKey`/`rpcPollSub`/`rpcSubmitX402` 激活后统一调 `rpcRefreshSub()` 进详情页；详情页 key 面板：本浏览器保存的明文可复制，否则展示掩码（明文仅签发一次），无 key 提供签发按钮；`rpcLoadStatus` 补充详情页 KPI「节点活跃 x/y 池」；tab 沿用 core.js 全局 `.tab-btn[data-sub]` 委托（`#page-rpc` 容器内），无新增绑定。playwright 本地实测：未连钱包 → intro 显示 connect CTA + skip 按钮，点击 skip 进入详情页并激活节点状态 tab ✓；模拟钱包 → 3 张套餐卡（RPC Free/Pro/Enterprise）渲染 ✓；注入 `rpcSubState` 调 `rpcLoadDashboard()` → 详情页 KPI（Free / 1,234 / 10,000）+ key 面板掩码展示 ✓；tab 切换 status/events/docs 全部激活（事件链下拉 10 链、Docs 5 段代码块）✓；缓存版本号 bump 1787300600。W-8e（节点状态改用户视角）：应反馈"节点状态 · RPC 池健康——这是给用户的界面，只需要展示我们提供的 RPC 端点的状态"——`rpcStatusHtml`/`rpcLoadStatus` 重构为只展示对外端点：面板标题改为「📊 节点状态 · 我们提供的 RPC 端点」，每链一行 = 链名 + chainId + `https://rpc-gw.0xainet.top/v1/rpc/<chain>` + 状态（池内 ≥1 healthy → ● 正常 / 有端点但全挂 → ● 不可用 / 无端点 → ● 未配置）；不再渲染内部 Endpoint key / provider / tier 明细；详情页 KPI 由「节点活跃 x/y 池」改为「可用链 x/y 链」；playwright mock 实测：3 链行、KPI 1/3 链、正则校验无 tenderly/provider/tier 泄露 ✓；缓存版本号 bump 1787301000。W-8f（链上事件下拉白底看不清修复）：`#rpc-ev-chain` 由内联 `background:var(--surface)` 改为 `.select` 类（对齐 DC 筛选下拉），并在 infrax.css 新增 `select option{background:var(--surface-input);color:var(--text-primary)}` 使原生下拉选项面板跟随深色主题；playwright 实测 computed style：select 与 option 均深底（rgb(26,32,38)）浅字（rgb(247,248,250)）✓；缓存版本号 bump 1787302000。
