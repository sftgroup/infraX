...
| 编号 | 需求 | 状态 | 优先级 | 备注 |
|---|---|---|---|---|
| WSG-1 | **Data & Insights 面板内升级入口**：dc-dash 新增"我的订阅"tab（`data-sub="dc-sub"`，复用 RPC/LightRAG 模式）——当前套餐 + 当月用量（`currentUsage/monthlyQuota` 进度条）+ 升级卡片 `dcUpgradeHtml`（基于当前 `dcPlan` 过滤更高档套餐，点击 `dcSubscribe(planId)` 走既有支付流程 chain/fiat/x402） | 🔲 待排期 | P1 | 涉及 index.html（dc-dash tab-row + sub-panel）、datacenter.js（`dcLoadMySub`/`dcUpgradeHtml`/`dcRefreshUsage` 复用）、i18n.js（新增 `dc_sub_*`/`dc_upgrade_*` zh/en 键 + check-i18n-keys 校验 + HTML `?v=` bump）；当前已订阅用户无升级路径（intro 订阅卡 `dcSubscribe` index.html L813-827 仅首次激活用） |
| WSG-2 | **全平台统一服务状态监控页**：独立 nav 页（`status`，新模块 `modules/status.js` + page div + core.js loader）——聚合全部服务真实 health 探针（参照 docs/INFRAX_MONITORING.md 24+ 端点清单：MPC `/api/v2/mpc/status`、Safe `/api/vault/safe/status`、WaaS `/api/v2/saas/tenants/my`、DC `/api/v2/data/usage`、RPC `/api/v2/rpc/health`、LightRAG `/api/rag/api/v1/health`、AA `/v1/plans`、data :9112 / ml :9120 / ragservicer :9721 等），每服务 状态+延迟+详情，KPI 汇总"正常 x/y"，手动刷新 + 30s 自动轮询，fail-silent 降级 | 🔲 待排期 | P1 | 涉及 index.html（nav 项 + 页面 + 脚本引入）、core.js（`PAGE_TITLES`/loaders/subLoaders）、新建 modules/status.js、i18n.js（`st_*` 键）bump；缺口背景：当前仅 Dashboard 首页聚合（index.html L116-128），无独立监控页、无服务级延迟/轮询 |
| WSG-3 | **MPC 页面用量展示**：mpc-wallet.js 页面补用量/计费模式展示（当前仅状态 `🟢 Active`，mpc-wallet.js L139-143）——复用 `/api/v2/mpc/plans`（mode/configured），展示计费模式 + ledger 余额（需会话 token 时降级展示模式）；或评估后确认 Dashboard 聚合行已覆盖则关闭 | 🔲 待排期 | P2 | 涉及 mpc-wallet.js、i18n.js；Dashboard 聚合行（nc-wallet.js L102-105）已展示模式，本项为页面内补齐 |
