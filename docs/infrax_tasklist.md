...
**MQ-10 实施 Tasks（链上 RPC 网关独立化，2026-08-08 方案定稿 → 本地开发完成，生产部署待排期）**：
...
- [ ] T-7 **配置收敛**：collector/mpc/dc 直连 RPC 改走网关 → 全仓单一 RPC 入口（后续可选）
- [x] T-8 **部署验证**：本地 E2E 通过（读 `0xae9464`、广播无效 rawTx 502、写方法 403、未授权 401、链别名/未知链 400）；systemd unit `deploy/systemd/infrax-chain-rpc.service` 就绪（生产部署 pending）
