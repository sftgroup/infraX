...
### 9.19 web 门户服务化缺口补齐（WSG，2026-08-23 全站审计提出）

> **审计结论（2026-08-23，对照 9.14 W-9 系列落地后现状）**：门户 9 板块（Dashboard/Chain RPC/LightRAG/WaaS/Data & Insights/MPC/Safe/AA/Payments）三项能力盘点——**① 升级套餐卡片**：RPC（`rpcUpgradeHtml` b2b.js L387）/ LightRAG（`lrUpgradeHtml` b2b.js L826）/ WaaS（`#waas-upgrade-panel` index.html L410）已具备，**Data & Insights 缺口**（订阅后 dc-dash 无升级入口）；**② 用量监控**：Dashboard 统一用量表（nc-wallet.js L89 `#dash-usage` 聚合 DC/MPC/WaaS/Safe/AA）+ RPC 每 key 进度条 + DC `dc-usage-count/quota` + LightRAG 真实文档数已具备，**MPC 页仅状态无用量（弱）**；**③ 统一服务状态监控页**：仅 Dashboard 首页 `#dash-services` 总览表（6 服务 active/inactive + RPC/LightRAG health 探针），**无独立全平台监控页**。以下三项补齐，待排期开工。

| 编号 | 需求 | 状态 | 优先级 | 备注 |
|---|---|---|---|---|
| WSG-1 | **Data & Insights 面板内升级入口**：dc-dash 新增"我的订阅"tab（`data-sub="dc-sub"`，复用 RPC/LightRAG 模式）——当前套餐 + 当月用量（`currentUsage/monthlyQuota` 进度条）+ 升级卡片 `dcUpgradeHtml`（基于当前 `dcPlan` 过滤更高档套餐，点击 `dcSubscribe(planId)` 走既有支付流程 chain/fiat/x402） | ✅ 已上线（commit 653faf0） | P1 | 涉及 index.html（dc-dash tab-row + sub-panel）、datacenter.js（`dcLoadMySub`/`dcUpgradeHtml`/`dcRefreshUsage` 复用）、i18n.js（新增 `dc_sub_*`/`dc_upgrade_*` zh/en 键 + check-i18n-keys 校验 + HTML `?v=` bump）；当前已订阅用户无升级路径（intro 订阅卡 `dcSubscribe` index.html L813-827 仅首次激活用）；验证：本地+生产 playwright 均渲染 套餐名/进度条/dx_ key/升级卡 |
| WSG-2 | **全平台统一服务状态监控页**：独立 nav 页（`status`，新模块 `modules/status.js` + page div + core.js loader）——聚合全部服务真实 health 探针（参照 docs/INFRAX_MONITORING.md 24+ 端点清单：MPC `/api/v2/mpc/status`、Safe `/api/vault/safe/status`、WaaS `/api/v2/saas/tenants/my`、DC `/api/v2/data/usage`、RPC `/api/v2/rpc/health`、LightRAG `/api/rag/api/v1/health`、AA `/v1/plans`、data :9112 / ml :9120 / ragservicer :9721 等），每服务 状态+延迟+详情，KPI 汇总"正常 x/y"，手动刷新 + 30s 自动轮询，fail-silent 降级 | ✅ 已上线（commit 653faf0） | P1 | 涉及 index.html（nav 项 + 页面 + 脚本引入）、core.js（`PAGE_TITLES`/loaders/subLoaders）、新建 modules/status.js（`STATUS_SERVICES` 9 项 + `statusInit`/`statusProbe`/`statusRefresh`）、i18n.js（`st_*` 键）bump；缺口背景：当前仅 Dashboard 首页聚合（index.html L116-128），无独立监控页、无服务级延迟/轮询；验证：本地+生产 playwright 9 行服务表 + KPI 正常 x/y |
| WSG-3 | **MPC 页面用量展示**：mpc-wallet.js 页面补用量/计费模式展示（当前仅状态 `🟢 Active`，mpc-wallet.js L139-143）——复用 `/api/v2/mpc/plans`（mode/configured），展示计费模式 + ledger 余额（需会话 token 时降级展示模式）；或评估后确认 Dashboard 聚合行已覆盖则关闭 | ✅ 已上线（commit 653faf0） | P2 | 涉及 mpc-wallet.js、i18n.js；Dashboard 聚合行（nc-wallet.js L102-105）已展示模式，本项为页面内补齐；实现：`mpcDash()` 末尾 `afetch('/api/v2/mpc/plans')` fail-silent，mode==='metered' 显示"计费模式: 按量计费"否则"免费"；验证：本地+生产 playwright `🟢 Active · 计费模式: 按量计费` |
| WSG-4 | **服务状态查询服务器缓存**（WSG-2 优化，2026-08-23 用户提出"服务器应该有缓存"）：web server.js 新增 `GET /api/v2/system/status`——内网并发探测 9 服务（每服务 3s 超时，`STATUS_PROBE_TIMEOUT_MS`）+ 进程内存缓存（`STATUS_CACHE_MS` 默认 30s，所有用户共享一轮探测，探测异常回退旧缓存）；前端 status.js 改请求单接口渲染（删 `statusProbe` 公网直连），展示顺序/判定逻辑不变 | ✅ 已上线（commit e20a515） | P2 | 涉及 server.js（`RAG_HOST/RAG_PORT` 默认 localhost:9721 + `STATUS_SERVICES` 9 项 + `probeService`/`getSystemStatus` + 路由）/ modules/status.js（`statusRefresh` 改单接口）；生产 43.163.105.172 drop-in `infrax-web.service.d/status.conf` 配置 `RAG_HOST=43.156.78.59` `RAG_PORT=9721` `ML_HOST=43.156.25.197` `ML_PORT=9120`；验证：本地 node --check + 生产 9 服务 4×200（RPC/LightRAG/Data/ML）+ 5×401/400（MPC/Vault/WaaS/DC/AA 在线）+ 缓存窗口内多次请求 updated_at 一致 |

### 9.20 AIHunter 上游问题跟踪回执（源：`INFRAX_UPSTREAM.md`，AIHunter 推送 3 项待确认，2026-08-23）

> **✅ 全部答复 + 502/504 修复 + SOL 链纳入（2026-08-23，commit c8c78a4 + 387576b + a147463 已部署生产）**：AIHunter 以 `INFRAX_UPSTREAM.md` 推送 3 项待确认（token/history count:0、间歇 502/504、dx_6d2a2d key 解冻），InfraX 侧第 7 节回执（见 [INFRAX_UPSTREAM.md](file:///home/steven/infraX/INFRAX_UPSTREAM.md)）。协作原则（13.5）：独立项目只通知不代修；问题以文档推送，本回执以文档答复。

| 编号 | 问题 | 结论 | 状态 | 优先级 | 备注 |
|---|---|---|---|---|---|
| AHR-1 | **token/history 对多数币 count:0** | **快照覆盖限制（非故障）**：数据来自 collector 自有表 `okx_market_token_profiles`，仅覆盖每链热门 top 30（`OKX_MARKET_CANDLE_TOKENS`）、5min 粒度；非榜单币无快照 → count:0。建议 AIHunter History tab 优先展示榜单币、非榜单币空态提示"无快照覆盖" | ✅ 已答复 | P1 | 数据源/链路证据见回执 7.1；不支持任意 address 按需回填 |
