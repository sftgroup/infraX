...
| EPF-9 | 全量去除 PocketX 品牌残留（414 处/80 文件）：T-0 展示命名已完成（commit 626460e）；T-1 文档注释（P2）→ T-2 SDK 包名（P1，通知 B 端）→ T-3 9 库改名（P0，停机窗口，**无需重建表**，ALTER DATABASE RENAME）；T-4 运行语义保留项（walletService namespace 派生 / tenantService @web3.pocketx.local 邮箱 / payment `PocketX auth` 签名消息 / EIP-712 domain）**永不改** | ⚠️ T-0 完成 | P0 | 方案文档 `docs/EPF-9_POCKETX_RENAME_PLAN.md`；T-3 核心结论：DB 改名不重建表，但 admin `${BASE}/pocketx_*` 硬拼接必须同步；collector 库在 10.3.8.6 需无写入窗口；`pocketx_app` DB 用户一并 `ALTER ROLE RENAME`；B 端通知文案见方案 §5 |
