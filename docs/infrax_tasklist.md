...
| EPF-9 | 全量去除 PocketX 品牌残留（414 处/80 文件）：T-0 展示命名（626460e）→ **T-1 文档注释（22 md/141 处 + 代码注释）→ T-2 SDK 包名（payment→infrax-payment，删冗余 pocketx-sdk.ts）→ T-4 运行语义 4 处加注释**（e53ecac，2026-08-22）→ T-3 9 库改名（P0，停机窗口，**无需重建表**，ALTER DATABASE RENAME）；T-4 保留项（walletService namespace 派生 / tenantService @web3.pocketx.local 邮箱 / payment `PocketX auth` 签名消息 / EIP-712 domain）**永不改** | ⚠️ T-0~T-2/T-4 完成，T-3 待停机窗口 | P0 | 方案文档 `docs/EPF-9_POCKETX_RENAME_PLAN.md`；T-3 核心结论：DB 改名不重建表，但 admin `${BASE}/pocketx_*` 硬拼接必须同步；collector 库在 10.3.8.6 需无写入窗口；`pocketx_app` DB 用户一并 `ALTER ROLE RENAME`；B 端无 `pocketx-*` 包依赖无需通知；aa-sdk 134 单测通过 |
