...
- [x] **MQ-10 补充 E-5（✅ 2026-08-08 完成首期，钱包模块 + 会话模块）MPC 独立 SDK（需求登记，PRD §4.5）**：MPC 为独立微服务 → 独立轻量 SDK，不依赖整包 `infrax-dk`。已发布 `@0xinfrax/mpc-sdk@0.1.0`（`projects/mpc-sdk/`，零运行时依赖），生产 E2E 22/22 全绿。拆分子任务：
  ...
  - [ ] **E-5d 链上模块（7 tools）**：`balance`（原生+ERC20）/ `signMessage` / `signTypedData` / `sendTransaction` / `contractRead` / `contractWrite` / `gasEstimate`；端点 `/api/v2/mpc/{balance,sign-message,sign-typed-data,send-transaction,contract-read,contract-write,gas-estimate}`；**服务端已 TSS 就绪（E-4①/M3，2026-08-09 四端点本地 E2E 13/13，见 E-2e）**，SDK 侧 7 tools 封装 🔲 后续版本（首期未含）
