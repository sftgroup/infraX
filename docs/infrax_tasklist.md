...
| DS-15 | **ML 因子历史回填**（对已上线 30 符号按历史 1d bars 回放推理写 `ml_predictions` / `raw_snapshots`，解决「回测含 ML 因子早期区间全空」） | 🔄 阶段一 ✅ | P1 | 脚本：ml-service `scripts/backfill_ml_history.py`（bolt/moirai 单变量·多变量回放、tree 逐日聚合、timesfm 单变量长上下文）+ data `scripts/ingest_backfill.py`（INSERT OR IGNORE 幂等落库，试点 3 行重插被正确跳过）。**阶段一完成（2026-08-08，不停服）**：bolt **+4853** / moirai **+4044**（2024-09-09 → 2026-08-07）+ tree **+1723** 个按日聚合快照（2023-10-06 → 2026-08-07），`ml_predictions` 现 9668 行 / 30 符号全覆盖；`/factors/history` 早期区间（2024-10-01）实测已含 tree/bolt/moirai 因子。**阶段二待办（timesfm）**：实测峰值 RSS **1.89Gi**、单次推理 ~0.7s、全量 ~4830 次 ≈ **1.5-2h**；3.6Gi 机不停服（服务 2.2Gi + 回填 1.89Gi）必吃满 swap → 需低峰**停服窗口**执行，落库验证后恢复服务 |
