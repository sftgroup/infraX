...
| GP-1 | 图谱端点服务端缓存（源：`projects/data/AITRADER_GRAPH_PERF_REQ.md` R1）：首次生成后对相同查询（symbol/namespace/limit）缓存结果（Redis 或磁盘）直接返回；实体/文档增量更新时保持旧图可用、后台重建 + 原子切换 | 🔲 待办 | **P1** | **2026-08-20 评估（建议优先）**：链路 AItrader → data-service :9112 `/factors/graph/*`（透传，`ml_client.fetch_graph_edges` 已有 30s 进程内缓存）→ ml-service :9120 `/ml/graph/edges`。**ml-service `graph_edges_endpoint`（main.py#L661-L686）未接入现有 `_endpoint_cache`（TTLCache，`ML_CACHE_TTL_SEC` 默认 1800s + `AsyncCacheRunner` + prewarm 预热均现成）**，直接读图快照 `_LAST_GRAPH`，冷态可能触发构建 → 首次 >60s。接入后即可消除冷态重复构建；data-service 30s 缓存 TTL 顺带对齐 |
...
| GP-3 | 图谱生成链路性能优化（R3）：embedding 结果缓存（同一文档/实体不重复向量化）、图谱分片/增量构建、LLM 并发/批处理 | 🔲 待办 | P2 | 需深入 `ml-service/app/graph_engine.py` 评估，供参考 |
| GP-4 | 图谱端点 SLA 承诺（R4）：缓存命中 P95<500ms、冷生成 P95<10s（R1/R2 落地后目标）、失败返回结构化错误（`503+reason`）替代 fail-silent | 🔲 待办 | P2 | 当前 `graph_edges_endpoint` fail-silent 返回空结构（main.py#L684-L686），AItrader 无法区分「生成中/故障」；R1 落地后定标 |
