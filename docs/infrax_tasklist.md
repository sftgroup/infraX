...
>
> MQ-10 收敛与优化 DC-1~DC-9 已全部完成并在生产验证（2026-08-08），见 §9.7 MQ-10 方案段。
...
- [x] **DC-3（✅ 2026-08-08 评估完成，维持暂缓）mpc 收敛**：审查确认 mpc `server.ts` 用 ethers `JsonRpcProvider` 单 URL（env `*_RPC_URL`）承载全部链上能力——原生/ERC20 余额、gas 估算、`eth_call` 只读、`Wallet.sendTransaction`/Contract 写广播（6 处调用点）。网关收敛冲突点：① ethers provider 单 URL 无法区分读/广播两把 key（网关读/广播分离端点），需拆「读 provider（fetch headers 带读 key → `/v1/rpc`）+ 广播独立调 `/v1/broadcast`」两层；② `wallet.sendTransaction` 的签名与广播耦合，拆层需重写调用点；③ mpc 无高可用诉求（非高频链上读）。结论：**暂缓**——待 mpc 有 HA 诉求或需要多端点容灾时再实施（收益场景：去掉自持 RPC key、端点自动降级）
...
- [x] **DC-8（✅ 2026-08-08 评估完成）新链类型接入评估**：现有链类型特殊分支盘点——① `rpcPoolConfig.ts` `normalizeChain` 别名表 + `ACTIVE_CHAINS` + 链 env URL 清单；② `whitelist.ts` `isReadMethod` 按 `chain==='solana'` 分流（SOLANA_READ_METHODS / EVM_READ_METHODS）；③ `rpcPool.ts` 5 处 EVM/Solana 特判：`getLatestBlock`（solana→getSlot）、`runHealthChecks`（solana→getHealth）、`broadcast`（solana→sendTransaction）、`waitReceipt`（solana→getSignatureStatuses）、`fetchSolanaBlocks`。评估结论：接入新链类型（如 Cosmos/Polkadot）需同步改 3-4 文件、每新增一类再添特判，当前可维护但会随链数增长恶化；**建议**在真正接入首条非 EVM/Solana 链时引入「链 profile」抽象（每链类型对象：healthMethod/latestBlockMethod/broadcastMethod/receiptMethod/readMethods，rpcPool 特判改为查表）；EVM+Solana 现状稳定，不做先行重构（避免过度设计）
