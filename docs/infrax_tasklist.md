...
| EPF-8 | `.env.production` 含生产明文密钥（ADMIN_PASSWORD/JWT_SECRET/CWALLET_API_KEY/DATABASE_URL 密码）且被 git 跟踪 | 🔲 待排期 | P2 | 历史遗留；建议迁移敏感项至 systemd drop-in + `.env.production` 改放 .gitignore 占位模板，rotate 密钥后统一 |
