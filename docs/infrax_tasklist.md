...
| DS-15 | **ML 因子历史回填**（对已上线 30 符号按历史 1d bars 回放推理写 `ml_predictions` / `raw_snapshots`，解决「回测含 ML 因子早期区间全空」） | ✅ | P1 | 脚本：ml-service `scripts/backfill_ml_history.py`（bolt/moirai 单变量·多变量回放、tree 逐日聚合、timesfm 单变量长上下文）+ data `scripts/ingest_backfill.py`（INSERT OR IGNORE 幂等落库）。**阶段一完成（2026-08-08，不停服）**：bolt **+4853** / moirai **+4044**（2024-09-09 → 2026-08-07）+ tree **+1723** 个按日聚合快照（2023-10-06 → 2026-08-07）。**阶段二完成（2026-08-08，停服窗口，实测 36min）**：timesfm **+4856** 行（2024-09-09 → 2026-08-07；峰值 RSS 1.89Gi，停服执行无 swap 压力，落库后服务已恢复）。全量收口：`ml_predictions` 现 **14524 行**（bolt 5126 / moirai 4294 / timesfm 5104）+ tree 快照 1781，30 符号全覆盖；`/factors/history` 早期区间（2024-10-01）实测已含 tree/bolt/moirai/timesfm 因子 |
...
- [x] ⑥ SDK 版本管理方式（PyPI/npm 发布 vs 仓库内引用）决策 — **✅ G-9 已发布**：npm `@0xinfrax/infrax-dk@0.4.0` 已发布 registry 验证（2026-08-08 新增 `infra.ml.*` ml-service 实时推理命名空间）；PyPI `lightrag-client 2.0.0` 构建 + twine check 通过，**延后发布（2026-08-08 用户决定）**——剩余步骤：PyPI token → `twine upload dist/*` → `pip install` 验证
...
**MQ-10 实施 Tasks（链上 RPC 网关独立化，2026-08-08 方案定稿，待排期）**：
- [ ] T-1 **服务骨架**：新建 `projects/chain-rpc/`（Express + tsx + config + logger + 统一契约鉴权），端口 :9005，web 代理 `/api/v2/rpc/*` → :9115
- [ ] T-2 **内核迁移**：迁移 collector `RpcPoolManager` + `rpcPoolConfig`（`INFRAX_RPC_POOL` env + `rpc-pool.json` + DB `admin_rpc_config` 合并）+ 补广播 `eth_sendRawTransaction` / Solana `sendTransaction`；链覆盖 sepolia/eth/base/bsc/oxa/Solana
- [ ] T-3 **端点**：`POST /api/v2/rpc`（任意 method 透传，契约对齐 waas `/api/v2/wallet/rpc`）、`POST /api/v2/rpc/broadcast`（广播返回 txHash）、`GET /api/v2/rpc/health`、`GET /api/v2/rpc/chains`（monitor 只读）
- [ ] T-4 **鉴权分级**：接入 `shared/auth-express` 统一契约；scope=**rpc**（读，普通 service key）/ **rpc_broadcast**（广播，独立 key）；data `api_keys` 表签发 rpc key（`rpc_` 前缀）；广播端点强制限流 + 白名单方法（仅 sendRawTransaction/sendTransaction）
- [ ] T-5 **waas 兼容**：`/api/v2/wallet/rpc` 保留钱包签名鉴权外层，内部 HTTP 转发到网关（`getRpcUrl` 4 链映射由网关接管）；SDK `wallet.rpc()` 行为不变
- [ ] T-6 **SDK**：infra-dk 新增 `infra.rpc` 命名空间（read/broadcast/health/chains），契约对齐网关
- [ ] T-7 **配置收敛**：collector 只读（fetchBlockRange/fetchLogs/getBalance）与 relayer 广播、mpc/dc 直连 RPC 改走网关 → 全仓单一 RPC 入口
- [ ] T-8 **部署验证**：生产部署（systemd + 代理 9115）+ 全链 read/broadcast E2E + 权限矩阵（读 key 广播→403）+ waas 兼容端点回归
