...
### 1.1 必须备份（不可再生，业务核心，总量 ~90MB）

| 库 | 内容 | 大小 | 备份优先级 |
|---|---|---|---|
| `pocketx_mpc` | MPC 钱包/密钥分片/验证码 | 9.2MB | **P0（丢了=用户资产不可恢复）** |
| `pocketx_payments` + `pocketx_payment` | 支付订单/账务流水 | 18MB | P0 |
| `pocketx_waas` / `pocketx_vault` / `pocketx_chainrpc` | 链上操作账本/多签/广播记录 | 27MB | P0 |
...
| 数据 | 理由 |
|---|---|
| `pocketx_collector.events`（~19G，库内 99% 体积） | 设计即 72h 保留自动删；可从链上 RPC 重扫恢复。**仅当需要 >72h 历史做回测/分析时才归档**（见 §3.4） |
| `okx_token_snapshots` / `market_usage` | 可从 OKX 重拉 / 统计损失可接受 |
| 日志、缓存 | 临时数据 |

> 备份口径结论：**全量 `pg_dump` 10 个业务库（collector 排除 events 表）+ 加密配置文件**，压缩后每日增量约 50~100MB，跨网传输无压力。
...
- 每个业务库独立 `pg_dump -Fc`（自定义格式，可单表恢复），collector 库加 `--exclude-table=events`
