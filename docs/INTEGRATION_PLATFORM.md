...
> ⚠️ **域名状态（2026-08-06）**：`infrax.0xainet.top` DNS 已切至 Cloudflare（A 104.21.21.11），但 `/api/*` 经公网返回 502（Cloudflare 回源配置待修正，见 [infrax_tasklist §2.1](./infrax_tasklist.md)）；origin `43.163.105.172` 直连全端点正常。
