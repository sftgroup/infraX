...
| Chain RPC 网关 | 9130 | **公网入口** `rpc-gw.0xainet.top`（标准 JSON-RPC 兼容） | rx_ 读 key / bx_ 广播 key（超限 503） | [chain-rpc.md](./chain-rpc.md) |
...
- **业务服务 key**：payments/vault/mpc 用系统 key（PAYMENTS_API_KEY / VAULT_API_KEY / MPC_API_KEY）；dc 用 `x-dc-api-key`（tenants 表签发）；market 用 `X-API-Key`（api_keys 表签发 pkx_ key）；chain-rpc 用 X-Service-Key（读/广播 key）或 `rx_`/`bx_` 订阅 key（读写分离，issue-key `kind` 参数签发）
...
4. **payments 仅内网；chain-rpc 已有公网入口**：payments 公网不可直达（外部集成方经 SDK 命名空间 `infra.payment`，配置 paymentsUrl 或业务服务转发）；chain-rpc 公网入口 `https://rpc-gw.0xainet.top`（2026-08-13，标准 JSON-RPC 兼容，ethers/viem 可直连）
