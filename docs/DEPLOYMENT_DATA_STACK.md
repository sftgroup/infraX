...
- [ ] **数据源 API key 申请清单（2026-08-06，需求登记）** 优先级：① Twelve Data https://twelvedata.com/pricing（外汇 6 对缺口，B 端验收唯一缺口）② FRED https://fred.stlouisfed.org/docs/api/api_key.html（宏观指标）③ NewsAPI https://newsapi.org/register（新闻）④ Alpha Vantage https://www.alphavantage.co/support/#api-key（DXY）⑤ Tiingo https://www.tiingo.com/account/signup（外汇/期货备选）；可选：CoinGecko/Adanos/CryptoCompare。已配置：Finnhub + Firecrawl。申请到 key 后写入生产 `.env`（可经 PUT /admin/config 热更新）
