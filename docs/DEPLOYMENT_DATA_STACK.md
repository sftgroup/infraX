...
- [ ] **外汇采集降频（2026-08-06，Twelve Data 限流衍生）** 现 `_collect_multi_market` 每 `KL_INTERVAL_SEC`（5 分钟）拉 6 个外汇对，Twelve Data 免费 tier 8 次/分钟 ~800 credits/天不够用 → 建议给 multi_kline 增加独立采集周期（如每 30 分钟或 1 小时拉外汇），或外汇只在新交易日增量拉取
