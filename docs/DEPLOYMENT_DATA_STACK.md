...
- [ ] **Tushare 接入（2026-08-06，需求登记）** B 端已提供 token（`17376847970` 账号 / `1dcf7b…d940`）。现状：代码无 tushare 集成（A 股/港股/日线走 akshare/腾讯，已可用）；tushare 为 HTTP POST + token 鉴权（非 API key 格式），需新增 provider（A 股财务/交易日历/指数成分增强）。可选：接入后 A 股数据质量提升；或保持现状 + 把 token 记入 `.env`（`TUSHARE_TOKEN`，注意白名单需加 `_DATA_KEY_FIELDS`）
