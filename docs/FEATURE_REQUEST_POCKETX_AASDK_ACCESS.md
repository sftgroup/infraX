...
- **状态**: ✅ **三项阻塞全部就绪（2026-08-16）**——① 已发布 `@0xinfrax/aa-sdk@0.1.0`；② entryPointAbi / parseBundlers 已导出（registry 产物验证通过）；③ 方案 A（MpcSigner 双模式）已实现并经 PocketX 确认。aa-relay 公网入口 `https://rpc-gw.0xainet.top/aa-relay/`（详见 `PAYMASTER_PROVISION_REQUEST.md` 六）。
...
- `@0xinfrax/session-key-core@0.2.1` 亦已发布（PocketX 误查无 scope 的 `session-key-core` 才 404；正确包名带 `@0xinfrax/` 前缀）
