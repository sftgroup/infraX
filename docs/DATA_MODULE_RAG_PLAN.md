...
| **Chronos-Bolt** | 单变量时序基础模型 | 零训练 | 快速点预测/概率基线（交叉验证） | 📌 P2 备选 |
| **Moirai 2.0** | 多变量时序基础模型 | 零训练 | 多资产联动/跨序列传导 | 📌 P2 备选 |
| **TimesFM 2.5** | 长上下文时序基础模型 | 零训练 | 16K 长历史点预测 + 置信区间 | 📌 P2 备选 |
...
| P2 模型 | 承担的分析任务 | 补的短板 | 资源预算 |
|---|---|---|---|
| Chronos-Bolt-small | 快速概率基线，与树模型方向交叉验证（分歧→降置信） | 树模型过拟合技术指标，缺独立"第二意见" | ~0.3G ✅ 两台均可 |
| Moirai-2-small | 跨资产联动信号（ETH 传导→BTC 联动风险） | 现有模型全为单标的 | ~0.5G ✅ |
| TimesFM-2.5-200m | 长周期趋势 + 置信区间（风险预算） | Kronos 上限 400-500 根 | ~0.8G ✅ |
| TimesFM-2.5 完整版(2.5B) | — | — | ~10G ❌ 两台均不可 |

> 承载结论：**主服务器可用 2.2G**、ml-service 可用 2.9G；三个轻量 P2 均可承载。推荐仍放 ml-service（架构单一、余量更大），轻量版可错峰加载。
...
| **43.156.25.197**（ml） | 2C/3.6G/swap 2G | ml-service :9120（五模型） | 常驻 223MB / 可用 2.9G |

- **常开结论**：P0+P1 五模型在 ml-service 一台常开无压力；P2 轻量版（Bolt-small/Moirai-small/TimesFM-200m）任意一个亦可加入错峰加载
